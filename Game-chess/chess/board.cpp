   ...
   //插入背景图片
    ...
    this->setPalette(palette);
      chesstype();
        ...
        // 创建联网按钮
        GPushButton * lw = new GPushButton(":/res/networking.png", ":/res/networking.png");
        lw->setParent(this);
        lw->move(465, 25);
        QString l = QString::fromUtf8("networking");
        lw->setToolTip(l);

        connect(lw, &GPushButton::clicked, [=](){
                   QString executablePath = "C:/Users/l1472/Desktop/chess1/Chess_boxed.exe";
                   // 使用 QProcess 启动可执行文件
   ...
   for(int i=0;i<8;i++)
    ...
    for(int i=0;i<8;i++)
    {
        for(int j=0;j<8;j++)
        {
            ...
            if(canSelect[i][j]==10)
            {
                p.drawImage(rect,QImage(":/res/canwalk.png"));
                canSelect[i][j]=0;
            }
        }
    }
